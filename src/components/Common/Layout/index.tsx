	...
	const handleSizeContainer = () => {
		if (width / 1.777777 < height) {
			return {
				...
				transform: `scale(${height / width / 1.777777 >= 1 ? width / 1.777777 / 1080 : width / 1.777777 / 1080})`,
			};
		} else {
			return {
				position: 'static',
				...
				height: height,
				transform: `scale(${height / width / 1.777777 >= 1 ? height / 1080 : height / 1080})`,
			};
		}
	};
		...
		<>
			...
			>
				<div className={`${styles.container}`} style={handleSizeContainer() as any}>
					...
					>
						<div className={`${styles.bodyGame}`}>{children}</div>
					</div>
				</div>
			</div>
		</>
